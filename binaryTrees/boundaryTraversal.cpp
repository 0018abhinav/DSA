...
void leafNodes(BTNode<int>*root){
    if(root==NULL)return;
...
void rightPart(BTNode<int>* root){
    if((root->left == NULL && root->right == NULL)||(root== NULL)){
        return;
    }
...
void boundaryTraversal(BTNode<int>* root){
    if(root == NULL) return;
    cout<<root->data<<" ";

    leftPart(root->left);
    leafNodes(root);
    rightPart(root->right); //reverse
}

...
//we will return pair which return height and bool of balanced or not, to reduce time complexity to O(n)
pair<bool,int> isBalanced(BTNode<int>* root){
    if(root == NULL){
    ...
    bool heightDiff = (abs(leftAns.second - rightAns.second)) <= 1;

    pair<bool,int> p;
    if(leftAns.first && rightAns.first && heightDiff){
        p.first = true;    
    }
    else{
        p.first = false;
...
//tree is considered to be balanced if the height diff of leftsubtree and rightsubtree is not more than 1 for every nodes
int main(){
